<template>
    <popup-modal size="lg" ref="popup">
        ...
        <p class="text-center px-4 mt-4">
            <span class="h6 d-inline-flex ml-2">{{ message }}</span>
        </p> 
        <div class="px-4 pt-2 pb-2">
            
            <div class="btns d-flex justify-content-end pb-2">
                <button class=" cancelll ml-3 bg-secondary" @click="_cancel">{{ cancelButton }}<i class="fa fa-folder-closed" aria-hidden="true"></i></button>
                <button class=" cancelll ml-3 bg-danger" @click="_confirm">{{ okButton }}</button>
            </div>
