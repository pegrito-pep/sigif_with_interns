                ...
                <b-overlay rounded="sm" :show="showOverlayTypestitre">
                    ...
                    >
                        <v-select @input="showTitre" @open="getTypestitres" :options="typesTitres" 
                            v-model="search.typetitre"  :reduce="elt => elt.code" 
                            label="label" class="custom-select-type_titre">
                        </v-select> 
                    </b-form-group>
                </b-overlay>
                    <div>
...
export default {
    ...
    methods: {
        ...
        postData(){
            this.wait=true;
            this.$jasper.post("titre-operationnels/pdf", {
                ...
                if(response.status==200) {
                    ...
                    link.download = 'Listing des titres opérationnels.pdf';
                    link.click();
        ...
        postData2(){
            this.wait=true;
            this.$jasper.post("ItextController/listeTitrePdf", {
                ...
                if(response.status==200) {
                    ...
                    const link = document.createElement('a');
                    link.href = URL.createObjectURL(blob);
                    link.download = 'Listing des titres valides.pdf';
                    link.click();
                    URL.revokeObjectURL(link.href)
                    this.wait=false;
                   this.resetData() //on re-initialise les champs du formulaire
