    ...
    methods: {
        ...
        async confirm() {
            ...
            this.content.map(elt =>{
                ...
                elt.nonusuel =elt.ESSENCE
                elt.longueur =elt.LONGUEUR
                elt.largeur =elt.LARGEUR
                elt.epaisseur =elt.EPAISSEUR
                elt.epaisseur =elt.EPAISSEUR
                elt.volume = this.setVolume(elt.LONGUEUR, elt.LARGEUR, elt.EPAISSEUR)
                elt.poids =elt.POIDS
                elt.nmbrpiece =elt.NBPIECES

                return elt;
            })
          
            /*for (let i = 0; i < this.content.length; i++) {
            const codebarre = this.content[i].codebarre

            if (!obj.hasOwnProperty(codebarre)) {
                obj[codebarre] = []
            }

            obj[codebarre].push(this.content.data[i])
            }

            const result = Object.values(obj)*/
    
          let productionDto={utb: 489, ligne: 7667, colisDtos: this.content}
          //let productionDto={utb: this.dto.utb, ligne: 7667, colisDtos: this.content.data}
            this.submitCodes(productionDto);
        },
        async submitCodes(data){
            this.submitted=true;

            try {
                await this.$transformation.post("productions/import_codebarre_lotProduction", data).then(response =>{
      ...
      //méthode de gestion e l'upload de fichier csv
        ...
        checkValidity(data){
            data.forEach(elt =>{
                if(php.empty(elt.CBCOLIS)||elt.CBCOLIS.length!==8){
