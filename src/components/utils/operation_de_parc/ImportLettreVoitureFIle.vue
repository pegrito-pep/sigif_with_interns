<template>
    <popup-modal size="lg" ref="popup">
        ...
        <div class="px-10 pt-2 pb-2">
            <div class="pl-2">
                <a href="#" class="link-import-file px-2"  @click="confirm"><b-spinner small v-if="submitted" class="ml-2"  style="margin-right: 5px;"></b-spinner> <span v-if="!submitted"> {{ okButton }}</span></a>
...
export default {
    ...
    methods: {
        ...
        async confirm() {
          ...
          let lettrevoitureDto={typelv: this.dto.typelv,sitedepart: this.dto.sitedepart,  sitedestination: this.dto.sitedestination}
          if(this.dto.typelv=='LVD'){
            lettrevoitureDto.debites = codebarres
          }
          else if(this.dto.typelv=='LVG'){
            lettrevoitureDto.grumes = codebarres
          }
          console.log('lvdto ',lettrevoitureDto);
          this.submitCodes(lettrevoitureDto);
            //this.resolvePromise(true)
        },
        async submitCodes(data){
            this.submitted=true;
            try {
                await this.$operationParc.post("lettres-voiture/import_codebarre_lettre_voiture", data).then(response =>{
                    this.result = response.data.result.Items
                    if(this.result!=undefined){
                        if(this.dto.typelv=='LVD'){
                        console.log('entrée 1 réponse débités');
                        return this.$emit('importsuccessfulldebites',this.result)
                        }
                        else if(this.dto.typelv=='LVG'){
                            console.log('entrée 1 réponse grumes');
                            return this.$emit('importsuccessfullgrumes',this.result)
                        }
                    }else{
      ...
      //méthode de gestion e l'upload de fichier csv
        ...
        checkValidity(data){
            data.forEach(elt =>{
                if(php.empty(elt.CODE_BARRE)||php.empty(elt.SEQUENCE)||php.empty(elt.NOM_COMMERCIAL)||php.empty(elt.ID_ESSENCE)
                ||php.empty(elt.NUM_DF10)||
                php.empty(elt.DIAMETRE_PETIT_BOUT)||php.empty(elt.DIAMETRE_GROS_BOUT)||php.empty(elt.LONGUEUR)
                ||elt.CODE_BARRE.length!==8){
