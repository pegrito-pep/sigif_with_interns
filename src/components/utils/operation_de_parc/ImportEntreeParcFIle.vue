...
export default {
    ...
    methods: {
        ...
        async confirm() {
          ...
          let entreeParcDto={site: this.dto.site,typeproduit: this.dto.typeproduit,date: this.dto.date, codebarres: codebarres}
          this.submitCodes(entreeParcDto);
            //this.resolvePromise(true)
        },
        async submitCodes(data){
            this.submitted=true;
            try {
                await this.$operationParc.post("entrees-parc/list_verif_import_entreeParc", data).then(response =>{
