<template>
    <data-modal size="sm" ref="popup">
        ...
        <div class="px-2 pt-2 pb-1">
            ...
            <b-row v-if="intitule!=undefined||numfacture!=undefined">
                <b-col>
                    <p><span class="text-uppercase">Intitulé: <b>{{ intitule }}</b></span></p> 
                    <p><span class="text-uppercase">N° de bordereau: <b>{{ numfacture }}</b></span></p>
                    <p><span class="text-uppercase">Oganisation: <b>{{ organisation }}</b></span></p>
                </b-col>
                    
                <b-col v-if="commande=='3'">
                    <b-row>
                        <b-col cols="5">
                            <label for="textarea-small">Motif de rejet:</label>
                          </b-col>
                          <b-col cols="7">
                            <b-form-textarea v-model="motifrejet" id="textarea-small" rows="4" size="sm" placeholder="raison pour laquelle vous emettez un rejet"></b-form-textarea>
                          </b-col>
            ...
            <div class="btns d-flex justify-content-center">
                ...
                <button class=" cancelll ml-3 bg-danger" @click="rejeter">{{ okButton }}
...
export default {
    ...
    data: () => ({
        submitted:false,
        idRenouvellement: undefined, // parametre permettant de spécifier l'ongle sur lequel la boite est utilisée
        // Parameters that change depending on the type of dialogue
        commande: undefined, // parametre permettant de spécifier l'ongle sur lequel la boite est utilisée
        title: undefined,
        message: undefined, // Main text content
        numfacture:undefined,
        intitule:undefined,
        organisation: undefined, // login
        okButton: undefined, // Text for confirm button; leave it empty because we don't know what we're using it for
        cancelButton: 'Fermer', // text for cancel button
        motifrejet: undefined, // parametre permettant de spécifier l'ongle sur lequel la boite est utilisée
        // Private variables
    ...
    methods: {
        show(opts = {}) {
            ...
            this.idRenouvellement=opts.idRenouvellement
            this.title = opts.title
            this.message = opts.message
            this.intitule = opts.intitule
            this.numfacture = opts.numfacture
        ...
        async rejeter() {
            this.submitted =true;
            try {
                await this.$donneesReference.put('sites/renouvellemnt/rejeter/'+this.idRenouvellement, { motifrejet: this.motifrejet })
                   .then((response) => {
                    App.notifySuccess(response.data.message)
                    return this.$emit('rejetAdmis',response)     
                })
            } catch (error) {
