<template>
    <popup-reject-modal :largeur="Number('800')" size="lg" ref="popup">
        <div class="header d-flex justify-content-between pb-3" style="height:50px;">
            <h2 class="mt-0 font-weight-bold " style="font-size: 2rem"> {{ title }}</h2>
            <slot name="close">
        ...
        <b-overlay rounded="sm" :show="showOverlayTypestitre">
            <slot name="content">
                <div class="p-2">
                        <b-row>
                            <b-col cols="4">
                                <span class="text-danger font-weight-bold" style="font-size:1.5rem">*</span>
                                <span class="font-weight-bold h5"> Type de titre</span> 
                            </b-col>
                            <b-col>
                                <v-select :options="typesTitresCatalogues" :class="{ 'styleToSet': notSetTypeTitre }" class="custom-select-type_titre  sigif-select" v-model="typetitre" label="libelle"></v-select>
                            </b-col>
                        </b-row>
                        <div v-if="typetitre!=null">
...
export default {
    ...
    data: () => ({
        ...
        typesTitresCatalogues:[
            {idtypeTitre: 1, libelle:"Autorisation d'enlèvement du bois", code:'AE'},
            {idtypeTitre: 2, libelle:"Autorisation de récupération du bois", code:'AR'},
            {idtypeTitre: 3, libelle:"Permis Spécial", code:'PS'},
            {idtypeTitre: 11, libelle:"Autorisation personnelle de coupe", code:'APC'},
            {idtypeTitre: 8, libelle:"Permis de bois d'oeuvre", code:'PEBO'},
        ],
        typetitre:null,
        wait:false,
    ...
    goToCreation(){
        console.log('entrée ici ',this.typetitre);
        this.$refs.popup.close()
        storage.set('typetitre',this.typetitre)
        setTimeout(() => {
            if(this.typetitre.code=='AE'){
                this.$router.push({ name: "create-catalogue-aeb"});
            }
            else if(this.typetitre.code=='AR'){
                this.$router.push({ name: "create-catalogue-arb"});
            }
            else if(this.typetitre.code=='PS'){
                this.$router.push({ name: "create-catalogue-ps"});
            }   
        }, 200);
    },
