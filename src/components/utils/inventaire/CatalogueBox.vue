    ...
    <popup-reject-modal :largeur="Number('800')" size="lg" ref="popup">
        ...
        <div class="px-2 pt-2 pb-1">
            ...
            <div class="btns d-flex justify-content-center">
                <button :disabled="submitted" class="cancell  mr-2" style="color: ;" @click="valider">
...
export default {
    ...
    methods: {
        show(opts = {}) {
            ...
            this.idcatalogue = opts.idcatalogue
        ...
        async valider() {
            this.submitted =true;
            try {
                if(this.commande=='validation'){
                    await this.$inventaire.put('catalogues/'+this.idcatalogue+'/valider')
                    .then((response) => {
                        this.submitted =false;
                        return this.$emit('validationcatalogueSet',response)  
                    })
                }
                else if(this.commande=='suppression'){
                    await this.$inventaire.put('catalogues/'+this.idcatalogue+'/supprimer')
                    .then((response) => {
                        this.submitted =false;
                        return this.$emit('suppressioncatalogueSet',response)  
                    })
                }
                ...
                if(error.response.data.code==403){
                    return App.alertError('Vous n\'ètes pas athorisé à effectuer cette action')
                }
