    ...
    <div id="cover" class="min-vh-100">
          ...
          <div class="m-0 px-2 table-header-border">
            <b-row>
              ...
              <b-col>
                <span v-if="size==0">Aucun feuillet sélectionné</span>
                <span v-else>{{size}} feuillet(s) sélectionné(s)</span>
              </b-col>
              <b-col class="col-md-auto">
                <!-- <b-button @click.prevent="addFeuillet" :disabled="!canCreateFeuillet" size="sm" :class="{'change-image-opacity': !canCreateFeuillet,  'not-change-image-opacity': canCreateFeuillet}" class="mx-1 simple-btn"><b-img  src="@/assets/images/iconPLUS_16x16.png"></b-img> Simulate</b-button> -->
                <b-button @click.prevent="imprimer" :disabled="!canPrint" size="sm" :class="{'change-image-opacity': !canPrint,  'not-change-image-opacity': canPrint}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconIMPRIMER_16x16.png"></b-img> Imprimer</b-button>
                <b-button v-if="takeAll" @click.prevent="imprimerTout" :disabled="!canCreateFeuillet" size="sm" :class="{'change-image-opacity': !canCreateFeuillet,  'not-change-image-opacity': canCreateFeuillet}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconIMPRIMER_16x16.png"></b-img> Décocher tout</b-button>
                <b-button v-else @click.prevent="imprimerTout" :disabled="!canCreateFeuillet" size="sm" :class="{'change-image-opacity': !canCreateFeuillet,  'not-change-image-opacity': canCreateFeuillet}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconIMPRIMER_16x16.png"></b-img> Imprimer tout</b-button>
          ...
          <div class="ml-1 w-75">
              ...
              <template v-slot:head(imprimer)="data">
                ...
                <template  #cell(imprimer)="data">
                  ...
                  <span style="display: flex;justify-content:center;align-items: center;">{{ data.item.imprimer }}</span>
                </template>
                <template  #cell(action)="data">
                  <!-- <a @click.prevent="handleDownload(data.item.numfeuillet)" class="xal d-flex justify-content-center align-items-center" v-b-tooltip.hover href="#"><b-img src="@/assets/images/Attach_128x128.png"></b-img>Télécharger</a> -->
                    <span style="display: flex;justify-content:center;align-items: center;"><input disabled type="checkbox" v-model="data.item.action" /></span>
                </template>
...
export default {
  ...
  data: () => ({
    ...
    fields: [ 
      ...
      { key: 'numfeuillet', label: 'N° du feuillet',thStyle: { width: "6%" } }, 
      { key: 'datecrea', label: 'Date de création',thStyle: { width: "5%" } },
      { key: 'codebarre', label: 'Code barre',thStyle: { width: "5%" } },
      { key: 'imprimer', label: 'Imprimer',thStyle: { width: "5%" } },
      { key: 'action', label: 'Action',thStyle: { width: "5%" } },
      { key: 'state', label: 'Etat',thStyle: { width: "5%" } },
    ],
    elements: [
      {
        numfeuillet:12,
        imprimer:'fichier 1.jpg',
        codebarre:'125400',
        action:false,
        'state':'Lancé'
      },
 ...
 methods: {
  ...
  imprimerTout(){
    this.elments=this.elements.map((e,i)=>{
      if(this.takeAll){
  ...
  onRowSelected(items) {
    ...
    if(this.selected.action){
      this.ids.push(this.selected.numfeuillet)
    }else{
      this.ids.pop()
    }
