<template>
    <div id="cover" class="min-vh-100">
            ...
            <b-collapse id="collapse-4" v-model="visible">
              <b-card class="b-card">
                <b-row>
                  ...
                  <b-col cols="3">
                    <sigif-form-group :label-cols="4" :label-cols-lg="5" label="Nom de classe de qualité"/>
                  </b-col>
                  <b-col cols="3">
                    <b-form-group   label-cols="4" label-cols-lg="2" label-size="sm" label="Produit" label-for="input-sm">
                        <v-select label="libelle"  :options="allEntitesForrestieres" class="custum-vselect-border" ></v-select>
                    </b-form-group>
                  </b-col>

                  <b-col cols="3">
                    <b-form-group  label-cols="4" label-cols-lg="5" label-size="sm" label="Type de produit" label-for="input-sm">
                        <v-select label="libelle"  :options="allEntitesForrestieres" class="custum-vselect-border" ></v-select>
                    </b-form-group>
                  </b-col>

                  <b-col class="b-col" cols="1">
          ...
          <div class="m-0 px-2 table-header-border">
            <b-row>
              <b-col><h4 class="tab-header-left-text"><b-img src="@/assets/images/picto_user_vert.png"></b-img> Liste des classes de qualités</h4></b-col>
              <b-col class="col-md-auto">
                <span >
                    <b-button :disabled="!canCreateDepartement" @click.prevent="addClasseQualite" size="sm" :class="{'change-image-opacity': !canCreateDepartement,  'not-change-image-opacity': canCreateDepartement}" class="mx-1 simple-btn"><b-img  src="@/assets/images/iconPLUS_16x16.png"></b-img>{{ $t('data.Créer') }}</b-button>
                <b-button :disabled="!canViewDetailsClasseQualite" size="sm" :class="{'change-image-opacity': !canViewDetailsClasseQualite,  'not-change-image-opacity': canViewDetailsClasseQualite}" class="mx-1 simple-btn"><b-img :class="{'change-image-opacity': !canViewDetailsClasseQualite}" src="@/assets/images/iconVISUALISER_16x16.png"></b-img>consulter</b-button>
                </span>
          ...
          <div class="ml-1">
              ...
              <paginator hr="top" :offset="offset" :total="total" :limit="perPage" :page="currentPage" @pageChanged="changePage" @limitChanged="(limit) => {perPage = limit}" />  
                 ...
                 <!--AJOUT D'UNE CLASSE DE QUALIE -->
          <b-modal id="ClasseQualite-form"  ref="ClasseQualite-form"  hide-footer
           ...
           header-text-variant="light">
              ...
              <div>
                  <classeQualite-form @ClasseQualiteAdded="handleAddDepartement" @ClasseQualiteAddedAndContinue="handleClasseQualiteAddedAndContinue"/>
              </div>
          </b-modal>
...
export default {
  ...
  data: () => ({
    ...
    fields: [
        { key: 'index', label: '' },
        { key: 'codequalite', label: 'code de classes de qualité' },
        { key: 'intitule', label: 'Nom de classe de Qualité' },
  ...
  computed:{
    ...
    canViewDetailsClasseQualite(){ return true; return this.hasAccess('CONSULTER_CLASSE_QUALITE')},
    canCreateClasseQualite(){return true; return this.hasAccess('CREER_CLASSE_QUALITE')},
...
getClasseDeQualite(){
     this.isBusy=true
      ...
      this.$donneesReference.get("qualites", {params}).then(response => {
        ...
        this.elements=response.data.result.items
         ...
         this.elements = this.elements.map(elt =>{
          elt.typeProduit = elt.produit.typeProduit
          elt.produit = elt.produit.intitule
          return elt
        })
	     this.isBusy=false
