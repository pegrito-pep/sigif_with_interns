  ...
  <div id="cover" class="min-vh-100">
    <b-overlay rounded="sm" :show="showOverlay">
                ...
                <div>
                  ...
                  <b-table show-empty hover  select-mode="single" responsive="sm" ref="selectableTabletransformation" selectable 
                    ...
                    <template #cell(codebarre)="data">
                      <a class="d-flex justify-content-center align-items-center" @click.prevent="traceCode(data.item.codebarre)" style="color:green">{{ data.item.codebarre }}</a>
        ...
        <error-dialogue ref="errorDialogue"></error-dialogue>
         <!--MODAL DE TRACAGE D'UN CODEBARRE RESULTAT-->
        <b-modal  header-class="custom-bg"
        header-text-variant="light"
        body-bg-variant="light" id="modal-xl" ref="modaltracabilite" size="xl" :title="title"   hide-footer>
          <trace-codebarre :codebarre="codebarremere"></trace-codebarre>
        </b-modal>
    </b-overlay>
...
import TraceCodebarre from "@/components/Tracabilite/TraceCodebarre.vue";
...
data: () => ({
  //data modal tracage codebarre mère
  title:'',
  codebarremere:'',
  showOverlay:false,
  visible:true,
...
methods: {
  traceCode(codebarre){
    this.title='Informations ressorties du codebarre:'+codebarre;
    this.codebarremere=codebarre
    this.$refs.modaltracabilite.show();
  },
  reset(){
