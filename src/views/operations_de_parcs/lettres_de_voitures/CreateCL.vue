<template>
  <div id="cover" class="min-vh-100">
              ...
              <b-overlay class="m-4 px-2" rounded="sm" :show="showOverlayCodeData">
                <div v-if="lettrev.typelv=='LVD'">
                  ...
                  <b-table height="50px" :items="itemsdetailsdebites"  :fields="fieldsdetailsdebites" class="mt-1" outlined   ref="selectableCodesDebites" select-mode="single" selectable @row-selected="onRowSelected" :tbody-tr-class="rowClass">
                    ...
                    <template v-slot:head(nmbpiece)="data">
                      <span v-html="data.field.label" style="color:green"></span>
                    </template>
     ...
     <annomalies-dialogue ref="annomaliesBox"></annomalies-dialogue>
...
import AnnomaliesDialogue from '@/components/utils/AnnomaliesDialogue.vue';
...
export default {
  ...
  methods: {
            ...
            if(commande='codebarre'){
              if(this.lettrev.typelv=='LVD'){
                ...
                .then(response =>{
                  ...
                  //this.elementsdebites.unshift(response.data.result);
    ...
    //TRAITEMENT CAS LETTRE DE VOITURE NON VIERGE
    ...
    this.$operationParc.post('lettres-voiture', this.lettrev).then(response => {
        ...
        this.submitted = false
        this.displayAnnomalies(error.response.data)
    })
},
    async sendAlertMessage(error){
        const ok = await this.$refs.codebarreinfo.show({
    ...
    async displayAnnomalies(error, index, fields){
        console.log('error ',error);
        const ok = await this.$refs.annomaliesBox.show({
          title: 'Information',
          message: error.message,
          anomalies: error.result.toString(),
        })
        //If you throw an error, the method will terminate here unless you surround it wil try/catch
        if (ok) {
            this.$refs.codebarreinfo._close();
