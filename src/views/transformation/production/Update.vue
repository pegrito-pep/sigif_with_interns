<template>
  <div id="cover" class="min-vh-100">
    <b-overlay rounded="sm" :show="showOverlay">
      <div>
        ...
        <b-row>
          <b-col>
            <h4 class="tab-header-left-text">
              <button id="toggleSideBar" @click="toggleSideBar"><span aria-hidden="true" style="font-size: 30px">&times;</span></button>
              <span class="font-weight-bold" v-if="lotproduction!=null">Modification du lot  n°{{ lotproduction.idlot }} </span>
            </h4>
          </b-col>
          <b-col class="col-md-auto">
            <span >
              <b-spinner small v-if="submitted" class="ml-2"  style="margin-right: 5px;"></b-spinner>
              <a v-else @click.prevent="sendFormEditionLot" :disabled="disable"  size="sm"   class="mx-1 px-1 simple-btn" style="cursor: pointer"><b-img src="@/assets/images/picto_enregistrer_vert.png"></b-img>Enregistrer</a>
              <a @click.prevent="fermer" size="sm" class="mx-1 simple-btn" style="cursor:pointer"><b-img src="@/assets/images/picto_retour_vert.png"></b-img>Fermer</a>
            </span>
          </b-col>
        </b-row>
        ...
        <!-- MODIFICATION LOT DE PRODUCTION-->
        <div>
          ...
          <div>
            <b-table  height="50px" :items="itemsentree"  :fields="fieldsEntreesUsinage" :busy="isBusy" class="ml-1 px-2 mt-1" outlined   ref="selectableTablegrumes" select-mode="single" selectable @row-selected="onRowSelectedEntree" :tbody-tr-class="rowClass">
              ...
              <template #cell(choisir)="data">
                <v-select v-model="data.item.choisir" @input="loadOtherData(0, data.item.choisir)"  :options="entreesLignes" :filterable="false" @search="onSearchLigne" label="idlot" class="custom-select-type_titre">
                ...
                <b-overlay :show="showOverlayCodeData" rounded="sm">
                  <b-table v-model="currentItemGrume"  height="50px" :items="itemsdetailsLot"  :fields="fieldsdetailsLot" :busy="isBusy" class="ml-4 px-2 mt-1" outlined   ref="selectableTableDetailLot" select-mode="single" selectable @row-selected="onRowSelectedDetailLot" :tbody-tr-class="rowClass">
                    ...
                    <template #cell(essence)="data">
                      <v-select :options="paginated" v-model="data.item.essence" :filterable="false" @search="onSearch" :reduce="essence => essence.idessence" label="nomusuel" class="custom-vselect-style" style="color:black!important; font-weight:bold">
            ...
            <div style="width:98%; max-height: 350px;">
              ...
              <b-overlay rounded="sm" :show="showOverlayCodeDatapieces">
                ...
                <template #cell(nbpiece)="data">
                  <input size="xs" class="w-75 mx-1 px-1 sigif-input-view text-left"  style="color:black!important" v-model="data.item.nbpiece" />
                </template>
      ...
      <lot-production-file-component ref="filecomponent" @importsuccessfull="handleDataImported" :dto="dto"></lot-production-file-component>
...
export default {
  ...
  components:{ LotProductionFileComponent,ErrorDialogue,NoActionPossibleDialogue, ChangeProductBox, CodeBarreInfo, ApiUnreachable},
   ...
   data: () => ({
    lotproduction:null, //lot de production à modifier
    ...
    fieldspieces:[
      ...
      { key: 'epaisseur', label: 'epaisseur(Cm)'}, { key: 'nbpiece', label: 'Nb pièces'},{ key: 'poids', label: 'Poids' },
    ...
    methods: {
      ...
      addFieldPiecesAssocies(){
        ...
        return this.elementspieces.unshift({codebarre: this.elementsdetailsLot[this.indexRowSelected].codebarre, longueur: '', largeur : '', epaisseur: '', nbpiece: '', volume: '', superficie:'', poids:''});
      ...
      async sendFormEditionLot() {

       
        //formattage des données au format demandé par l'api

        //formattage idpiece
        this.elementspieces.map(elt =>{
          elt.idpiece = elt.id!=null?elt.id:0
          elt.nombrepiece = elt.nbpiece
        })
        this.elementsdetailsLot.map(elt =>{
          elt.idcolis = elt.idcolis!=null?elt.idcolis:0
          elt.idessence = elt.idessence!=null?elt.idessence:elt.essence
          elt.reference = elt.sequence
          elt.numcolis = elt.numcolis
          return elt;
        })
      ...
      this.elementsentreeusinage.map(elt =>{ 
        ...
        this.$transformation.patch('productions/' +this.$route.params.id+'/modifier', this.form).then(response => {
      ...
      async getProductionData(){
        ...
        if(!this.$store.state.typesproduits){
          ...
          try {
            ...
            this.$store.dispatch('produitsTransformations', this.typesproduits)
          } 
          catch (error) {
              if (!error.response) {
                  // network error
   ...
   //infos détails lot de production choisi
   async getDetailsLotproduction() {
      ...
      console.log('lot de production ',this.lotproduction);
      this.form.utb=this.lotproduction.idsite
      this.form.ligne=this.lotproduction.usinage[0].ligne
      this.form.typeproduit=this.lotproduction.idtypeproduit
      this.form.dateproduction=this.lotproduction.dateoper!=null?this.lotproduction.dateoper.replace(/\//g, '-'):''
      this.form.numcontrat=this.lotproduction.numcontrat
      await this.getProductionData();
      if(!php.empty(this.lotproduction.usinage)){
        this.elementsentreeusinage = this.lotproduction.usinage
       
        console.log('éléments usinage ',this.elementsentreeusinage);
      }
      this.elementsdetailsLot=this.lotproduction.details
      this.elementsdetailsLot = this.elementsdetailsLot.map(elt => {
        elt.volume = elt.volume!=''? elt.volume.toFixed(3): 0
        return elt
      })
      this.elementspieces=this.elementsdetailsLot[0].pieces
    },
  },
  async beforeMount() {
    await this.getDetailsLotproduction()
    //await this.getProductionData()
  },
