        ...
        <div>
          <b-row>
            <b-col>
              <h4 class="tab-header-left-text">
                ...
                <span class="font-weight-bold" v-if="ligneamodifier!=null">Modification ligne de transformation n°{{ ligneamodifier.idligne }} </span>
              </h4>
            </b-col>
          ...
          <div>
            <b-row class="m-4 d-flex justify-content-around">
              <b-col cols="6">
                  ...
                  <b-row class="m-2">
                    ...
                    <b-col cols="7" v-if="ligneamodifier!=null">
                  ...
                  <b-row class="m-2" v-if="ligneamodifier!=null">
                    <b-col cols="4">
                    ...
                    <b-col cols="7">
                      <v-select v-model="form.lignetransformation"  :options="produitsTransformations" :filterable="false" :reduce="produit  => produit.IDPRODUIT" label="INTITULE" class="custom-select-type_titre" :class="{'styleToSet': notSetSiteoper }"></v-select>
                      <!--<select  v-model="ligneamodifier.typeTransformation" class="m-0 p-0" :class="{ 'styleToSet': notSetLignetransformation }"  style="width: 100%;font-size:1rem; border-radius:0.25rem; height: calc(1em + .300rem 2px) !important;">
              ...
              <b-col cols="4" v-if="ligneamodifier!=null">
                <b-form-group>
            ...
            <div class="w-75 mx-4 px-4">
              ...
              <b-table height="50px" class="mx-1 mt-1"  :items="items"  :fields="fieldsEsssence" :busy="isBusy"  outlined   ref="selectableTable" select-mode="single" selectable @row-selected="onRowSelected" :tbody-tr-class="rowClass">
                ...
                <template #cell(nomusuel)="data">
                  <v-select :options="paginatedEssences" :reduce="essence  => essence.IDESSENCE " v-model="data.item.nomusuel"  :filterable="false" @search="onSearch" label="NOMUSUEL" style="max-width: 400px!important" class="custom-select-type_titre">
...
export default {
    ...
    components:{ ErrorDialogue,NoActionPossibleDialogue, AlertBox, CodeBarreInfo },
     ...
     data: () => ({
      ligneamodifier:null, //ligne de transformation à modifier
    ...
    methods: {
      ...
      AddField () {
        ...
        if (php.empty(this.form.lignetransformation)) {
          this.notSetTpeProduit=true;
          let error={ message:'Veuillez choisir le type de prduit'}
          return this.sendAlertMessage(error);
        }
      ...
      async sendFormLigne() {
        ...
        this.elements.forEach(field => {
          this.form.rendements.push([isNaN(field.nomusuel)?field.idessence:field.nomusuel, parseFloat(field.rendement)])
        }) 
        if (php.empty(this.form.utb)) {
          this.notSetSiteoper=true;
       ...
       console.log('form final ',this.form);

        this.$transformation.patch('lignes-transformation/' +this.$route.params.id+'/modifierLigne', this.form).then(response => {
      ...
      async errorHappened(error, index){
        let message=error!=undefined ?error.message :'erreur interne du serveur'
      ...
      async getDetailLigneTransformation(){
        this.showOverlay = true
        this.ligneamodifier = await this.$transformation.get('lignes-transformation/' +this.$route.params.id).then(response => response.data.result)
        console.log('this.ligneamodifier',this.ligneamodifier)
        this.elements=this.ligneamodifier.essences
        this.form.utb=this.ligneamodifier.idsiteutb
        this.form.lignetransformation=this.ligneamodifier.idtypetransformation 
        this.form.nomligne=this.ligneamodifier.intitule
        this.form.description=this.ligneamodifier.description
        this.elements.forEach(field => {
            this.form.rendements.push([field.idessence, parseFloat(field.rendement)])
        })
    ...
    async beforeMount() {
      await this.getDetailLigneTransformation();
      await this.getLigneData()
    },
