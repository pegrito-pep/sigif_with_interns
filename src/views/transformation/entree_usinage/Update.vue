        ...
        <b-row>
          <b-col>
            <h4 class="tab-header-left-text">
              ...
              <span class="font-weight-bold" v-if="entreeusinage!=null">Modification entreeusinage  n°{{ entreeusinage.idlot }} </span>
            </h4>
          </b-col>
          <b-col class="col-md-auto">
            <span >
              <b-spinner small v-if="submitted" class="ml-2"  style="margin-right: 5px;"></b-spinner>
              <a v-else @click.prevent="sendFormEditionEntreeUsinage" :disabled="disable"  size="sm"   class="mx-1 px-1 simple-btn" style="cursor: pointer"><b-img src="@/assets/images/picto_enregistrer_vert.png"></b-img>Enregistrer</a>
          ...
          <b-row class="mx-4 d-flex justify-content-around">
            <b-col cols="6">
              <b-row class="m-2">
                ...
                <b-col cols="7" v-if="entreeusinage!=null">
                  <v-select v-model="form.utb"  @change="getLignes"  :options="paginatedSites" :filterable="false" :reduce="site  => site.IDSITEOPER" @search="onSearchSite" label="INTITULE" class="custom-select-type_titre" :class="{ 'styleToSet': notSetUtb }">
...
export default {
  ...
  components:{ ErrorDialogue,NoActionPossibleDialogue, ChangeProductBox, CodeBarreInfo,FileComponent},
   ...
   data: () => ({
    ...
    methods: {
      async sendFormEditionEntreeUsinage() {
        ...
        // console.log('données tableau 2', this.form.rendements);
          ...
          this.submitted =true;
          this.$transformation.patch('entrees-usinage/' +this.$route.params.id+'/modifier', this.form).then(response => {
    ...
    async getDetailsEntreeUsinage() {
      this.showOverlay = true
      this.entreeusinage = await this.$transformation.get('entrees-usinage/' +this.$route.params.id).then(response => response.data.result)
     console.log('entrée usinage ',this.entreeusinage);
      this.form.typeproduit=this.entreeusinage.typeprodut
      this.form.utb =this.entreeusinage.lotentEntete.siteoper
      this.form.lignetransformation =this.entreeusinage.lotentEntete.idligne
      this.form.dateoper =this.entreeusinage.dateoper.replace(/\//g, '-')
      await this.getEntreeUsinageData()
      console.log('form ',this.form);

      if(this.form.typeproduit=='GR'){
        this.elementsgrumes=this.entreeusinage.details
        this.elementsgrumes = this.elementsgrumes.map(elt => {
          elt.nomcommercial =elt.essence
          elt.diampb =elt.diametrepb
          elt.diamgb =elt.diametregb
          elt.volume = elt.volume!=''? elt.volume.toFixed(3): 0
          return elt
        })
      }
      else{
        this.elements=this.entreeusinage.details
  ...
  async beforeMount() {
    await this.getDetailsEntreeUsinage();
  },
