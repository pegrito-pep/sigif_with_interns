        ...
        <b-overlay :show="showOverlay" rounded="sm">
            <div class="m-0">
                <tab-head
                    ...
                    :nouveau="nouveau"
                    :canAddNew="canAddNew"
                    title="Secteur de la forêt communautaire n°"
                    :idElement="idElement"
                >
                </tab-head> 
                    ...
                    <b-row>
                        <b-col cols="7">
                            ...
                            <div>
                                ...
                                >
                                    <template #label>
                                        <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Secteur n°
                                    </template>
                                ...
                                >   
                                    <template #label>
                                        <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Superficie du secteur
                                    </template>
                                ...
                                <b-form-group class="p-0 m-0"
                                    label-cols="6"
                                    label-align="right"
                                    v-if="showNewForm && showFirst"
                                >
                                    <template #label>
                                        <span  style="color:red;font-weight: bold;">*</span>Année d'ouverture
                                    </template>
                                    <input type="text"  v-model="form.newyear"  class="w-15 mt-2  default-old-system-input-properties" :class="{'styleToSet':notSetFirstYear}" />
                                ...
                                <b-form-group v-else class="p-0 m-0"
                                    label-cols="6"
                                    label-align="right"
                                >
                                    <template #label>
                                        <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Année d'ouverture
                                    </template>
                                ...
                                >
                                    ...
                                    <a class="mx-1 detail-permis-item" v-else-if="selected.limeteufe!=='null'"   target="_blank" :href="path+selected.limeteufe"><b-img src="@/assets/images/Attach_128x128.png"></b-img> {{selected.limeteufe}}{{ selected.limeteufe ? (!(selected.limeteufe.includes('.')) ? '.pdf':'') :''}}</a>
                                    <span class="mx-1" v-else><b-img src="@/assets/images/Attach_128x128.png"></b-img> Aucun document</span>
                                </b-form-group>
                                <b-form-group class="p-0 m-0"
                                    label-cols="6"
                                    label-align="right"
                                >
                                    ...
                                    <a class="mx-1 detail-permis-item" v-else-if="selected.planquinquenal!=='null'"   target="_blank" :href="path+selected.planquinquenal"><b-img src="@/assets/images/Attach_128x128.png"></b-img> {{selected.planquinquenal}}{{ selected.planquinquenal ? (!(selected.planquinquenal.includes('.')) ? '.pdf':'') :''}}</a>
                                    <span class="mx-1" v-else><b-img src="@/assets/images/Attach_128x128.png"></b-img> Aucun document</span>
                                </b-form-group>
                            </div>
                                        ...
                                        <b-row>
                                            <b-col>
                                                ...
                                                >
                                                    ...
                                                    <a class="mx-1 detail-permis-item" v-else-if="selected.documentPBA!=='null'" v-b-tooltip.hover  target="_blank" :href="path+selected.documentPBA"><b-img src="@/assets/images/Attach_128x128.png"></b-img> {{selected.documentPBA}}{{ selected.documentPBA ? (!(selected.documentPBA.includes('.')) ? '.pdf':'') :''}}</a>
                                                    <span class="mx-1" v-else><b-img src="@/assets/images/Attach_128x128.png"></b-img> Aucun document</span>
                                                </b-form-group>
                                            </b-col>
                                        </b-row>
                                        ...
                                        <b-row>
                                            <b-col>
                                                ...
                                                >
                                                    ...
                                                    <a class="mx-1 detail-permis-item" v-else-if="selected.documentPGQ!=='null'" v-b-tooltip.hover  target="_blank" :href="path+selected.documentPGQ"><b-img src="@/assets/images/Attach_128x128.png"></b-img> {{selected.documentPGQ}}{{ selected.documentPGQ ? (!(selected.documentPGQ.includes('.')) ? '.pdf':'') :''}}</a>
                                                    <span class="mx-1" v-else><b-img src="@/assets/images/Attach_128x128.png"></b-img> Aucun document</span>
                                                </b-form-group>
                                            </b-col>
                                        </b-row>
...
export default{
    ...
    data(){
        return{
            ...
            fields:[
                ...
                {key:'numufe',label:'Secteur n°',thStyle:{color:'#4c9a2a'}},
                ...
                {key:'superficieufe',label:'Superficie du secteur',thStyle:{color:'#4c9a2a'}},
    ...
    //   },
      ...
      "form.newyear"(value){
        if(!php.empty(value)){
            this.notSetFirstYear=false
        }else{
            this.notSetFirstYear=true
        }
      },
  },
    methods:{
        ...
        async enregistrer(){
            ...
            if(this.showNewForm){
                if(php.empty(this.form.numUfe)){
                    this.notSetNumUfe=true
                    let error={ message:'Veuillez renseigner le secteur'}
                    this.showOverlay=false
                    return this.sendAlertMessage(error);
                }
                if(php.empty(this.form.supUfe)){
                    this.notSetSupUfe=true
                    let error={ message:'Veuillez renseigner la superficie du secteur'}
                    this.showOverlay=false
                    return this.sendAlertMessage(error);
                }
                if(this.showFirst && php.empty(this.form.newyear)){
                    this.showFirst=true
                    let error={ message:'Veuillez renseigner la date d\'ouverture'}
                    this.showOverlay=false
                    return this.sendAlertMessage(error);
                }
                if(php.empty(this.form.limites)){
                    this.notSetLimitesUfe=true
                    let error={ message:'Veuillez renseigner les limites du secteur'}
                    this.showOverlay=false
                    return this.sendAlertMessage(error);
                }
                ...
                if(this.showFirst){
                    this.form.lastYear=Number(this.form.newyear) + 4
                }
            }
            try{
                let superficies=[]
                ...
                const data={// qui est l'id du titreUfa obligatoire
                ...
                await this.$titres.post('foretCommunautaire/secteur',{
                    ...
                    this.showOverlay=false
                    let message={ message:'Secteur créé avec succès'}
        ...
        async getInitialData() {
            ...
            this.titreDetailsAac = await this.$titres.get('foretCommunautaire/secteur/' +this.$route.params.id)
                .then(response => {
            ...
            if(!php.empty(this.titreDetailsAac.secteurs)){
                this.isUfa=true
                
                this.numUfas2=this.titreDetailsAac.secteurs.map(elt=>{
                    return{
                        idsecteur:elt.idsecteur,
                        numufe:elt.numufe
                    }
                })
                // console.log('numufes',this.numUfas2)
                // this.selectedUfa = this.titreDetailsAac.titreufa.find(elt=>elt.numUfa===this.numUfas2[0].numUfa)
                
                this.numUfa=this.numUfas2[0].numufe
                console.log('numufe',this.numUfas2[0].numufe)
                ...
                // this.selectedUfa=this.titreDetailsAac.secteurs[0]
                this.selectedAac=this.titreDetailsAac.secteurs
                if(!php.empty(this.selectedAac)){
                    ...
                    // console.log('datePBA',this.selected.datePBA)
                ...
                const titresAac=this.selectedAac.map(elt=>{
                    return{
                        numufe:elt.numufe,
        ...
        onRowSelected(items) {
            ...
            if(!this.showNewForm){
                ...
                this.selectedAac = this.titreDetailsAac.secteurs.find(elt=>elt.numufe===items[0].numufe && elt.anneOuverture===items[0].anneeouverture)
                console.log('selectedUfa',this.selectedAac)
                // this.selected=this.selectedAac.find(elt=>elt.numufe===(items[0].numufe))
                this.selected=this.selectedAac
