                    ...
                    <b-row>
                        <b-col cols="7">
                            ...
                            <div>
                                ...
                                >
                                    <template #label>
                                        <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Secteur
                                    </template>
                                    <select  v-model="form.numufe" @change="showSupUfa" v-if="showNewForm" class="w-15 mt-2  default-old-system-input-properties" :class="{'styleToSet':notSetNumUfa}">
                                        <option v-for="(elt,i) in numUfas2" :key="i" :value="elt.idufe">{{ elt.numufe }}</option>
                                    </select>
                                    <select v-else disabled class="w-15 mt-2  default-old-system-input-properties" style="background-color:#eee;">
                                        <option selected :value="selectedUfa ? selectedUfa.idufe:''">{{ selectedUfa ? selectedUfa.numufe:'' }}</option>
                                    </select>
                                    <!-- <label class="label-form" v-else>{{ titreDetailsUfa ? titreDetailsUfa.numufa :'' }}</label> -->
                                
                                </b-form-group>
                                <b-form-group class="p-0 m-0"
                                    label="Superficie du secteur"
                                    label-cols="3"
                                    label-align="right"
                                >
                                    <input type="number" disabled v-model="form.supufe" v-if="showNewForm" class="w-15 mt-2  default-old-system-input-properties" />
                                    <input disabled type="text" v-else :value="selectedUfa?(selectedUfa.superficieUfe):''" class="w-15 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                ...
                                >
                                    ...
                                    <input v-else  type="text" disabled :value="selected.numAac" class="w-15 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                    <!-- <label class="label-form" v-else>{{ titreDetailsUfa ? titreDetailsUfa.numufa :'' }}</label> -->
                                </b-form-group>
                                <b-form-group class="p-0 m-0"
                                    label-cols="3"
                                ...
                                >
                                    <template #label>
                                        <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Superficie parcelle
                                    </template>
                                    ...
                                    <input v-else  type="text" disabled :value="selected.superficie" class="w-15 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                    <!-- <label class="label-form" v-else>{{ titreDetailsUfa ? titreDetailsUfa.numufa :'' }}</label> -->
                                </b-form-group>
                                <b-form-group class="p-0 m-0"
                                ...
                                >
                                    ...
                                    <input v-else  type="number" disabled :value="selected.anneExploittion" class="w-15 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                    <!-- <label class="label-form" v-else>{{ titreDetailsUfa ? titreDetailsUfa.numufa :'' }}</label> -->
                                </b-form-group>
                            </div>
                    ...
                    <b-row class="mt-0 p-0">
                        <b-col cols="6">
                            <b-table v-model="currentItemUfa"  ref="selectableTableufa" select-mode="single" responsive="sm" hover  selectable 
                                ...
                                <template #cell(parcelle)="data">
                                    <span>{{ formatAmount(data.item.parcelle) }}</span>
                                </template>
                                <template #cell(superficie)="data">
...
export default{
    ...
    methods:{
        ...
        showSupUfa(){
            ...
            console.log('numUfa',this.form.numufe)
            // this.numUfa=this.numUfas2.find(elt=>elt.idUfa===this.form.numUfa).numUfa
            // console.log('numUfa',this.numufa)
            this.form.supufe=this.titreDetailsAac.titreufe.find(elt=>elt.idufe===this.form.numufe).superficieUfe
        ...
        async enregistrer(){
            ...
            if(this.showNewForm){
                ...
                if(this.elementssuperficie2.length===0 || this.elementssuperficie2.some(x=>php.empty(x.arrondisement) && php.empty(x.superficie))){
                    let error={ message:'Veuillez renseigner la localisation de la parcelle'}
                    this.showOverlay=false
                    return this.sendAlertMessage(error);
                }
            }
            try{
                ...
                if(!php.empty(this.elementssuperficie2)){
                ...
                const data={
                    numUfe:Number(this.form.numufe),// qui est l'id du titreUfa obligatoire
        ...
        async getInitialData() {
            ...
            this.titreDetailsAac = await this.$titres.get('foretCommunautaire/parcelle/' +this.$route.params.id)
                .then(response => {
            ...
            if(php.empty(this.titreDetailsAac.titreufe)){
                this.isUfa=false
                let error={ message:'Ce titre n\'a pas de secteurs'}
                this.showOverlay=false
            ...
            this.numUfas2=this.titreDetailsAac.titreufe.map(elt=>{
                return{
                    idufe:elt.idufe,
                    numufe:elt.numufe
                }
            ...
            this.selectedUfa = this.titreDetailsAac.titreufe.find(elt=>elt.numufe===this.numUfas2[0].numufe)
            console.log('selectedUfa',this.selectedUfa)
            this.selectedAac=this.selectedUfa.titreaac
            ...
            this.selected.numAac=this.selectedAac[0].parcelle
            this.selected.annee=this.selectedAac[0].anneExploittion
            this.selected.superficie=this.selectedAac[0].superficieParcelle
            console.log('selectedufa',this.selectedUfa)

            const objUfas=this.titreDetailsAac.titreufe.find(elt=>elt.numufe===this.selectedUfa.numufe)
            const titresAac=objUfas.titreaac.map(elt=>{
                return{
                    numufe:this.selectedUfa.numufe,
                    parcelle:elt.parcelle,
                    superficie:elt.superficieParcelle
                }
            })
            // this.elementsufa.push({
            //     numufe: this.selectedUfa.numufe,
            //     parcelle:this.selected.numAac,
        ...
        onRowSelected(items) {
            ...
            if(!this.showNewForm){
                ...
                this.selectedUfa = this.titreDetailsAac.titreufe.find(elt=>elt.numufe===items[0]?.numufe)
                console.log('selectedUfa',this.selectedUfa)
                this.selected=this.selectedUfa.titreaac.find(elt=>elt.parcelle===Number(items[0].parcelle))
                console.log('selected',this.selected)
                if(this.selected){
                    this.selected.numAac=items[0]?.parcelle
