          ...
          <div class="m-0 px-2 table-header-border">
            <b-row>
              <b-col>
                ...
                <a :disabled="canCreateUtilisateur" @click.prevent="showSecteurs" size="sm" :class="{'change-image-opacity': !canViewDetailsSecteurs,  'not-change-image-opacity': canViewDetailsSecteurs}" class="mx-1 simple-btn"><b-img  src="@/assets/images/Picto_tracabilite/Parcs_Rupture_16x16.png"></b-img> Secteurs</a>
                <a :disabled="canCreateUtilisateur" @click.prevent="showParcelles" size="sm" :class="{'change-image-opacity': !canViewDetailsParcelles,  'not-change-image-opacity': canViewDetailsParcelles}" class="mx-1 simple-btn"><b-img  src="@/assets/images/Picto_tracabilite/Parcs_Usine_16x16.png"></b-img> Parcelles</a>
                <!-- <a :disabled="canCreateUtilisateur" @click.prevent="addUser" size="sm" :class="{'change-image-opacity': !canCreateUtilisateur,  'not-change-image-opacity': canCreateUtilisateur}" class="mx-1 simple-btn"><b-img  src="@/assets/images/Picto_tracabilite/iconABATTAGE_16x16.png"></b-img> AAC</a> -->
                <a :disabled="canCreateUtilisateur" @click.prevent="" size="sm" :class="{'change-image-opacity': !canViewDetailsCCP,  'not-change-image-opacity': canViewDetailsCCP}" class="mx-1 simple-btn"><b-img  src="@/assets/images/Picto_tracabilite/iconINVENTAIRE_16x16.png"></b-img> Contrat de partenariat</a>
              </b-col>
              <b-col class="col-md-auto">
                <span class="d-flex">
                <a :disabled="!canCreateConcessionDefinitive" @click.prevent="goToCreateTitreFCD" size="sm" :class="{'change-image-opacity': !canCreateConcessionDefinitive,  'not-change-image-opacity': canCreateConcessionDefinitive}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconPLUS_16x16.png"></b-img>Creer</a>
                <a :disabled="!canUpdateConcessionDefinitive" @click.prevent="showDetails" size="sm" :class="{'change-image-opacity': !canUpdateConcessionDefinitive,  'not-change-image-opacity': canUpdateConcessionDefinitive}" class="mx-1 simple-btn"><b-img :class="{'change-image-opacity': !canUpdateConcessionDefinitive}" src="@/assets/images/iconMODIFIER_16x16.png"></b-img>Modifier</a>
                <a :disabled="!canDeleteConcessionDefinitive" @click.prevent="showDetails" size="sm" :class="{'change-image-opacity': !canDeleteConcessionDefinitive,  'not-change-image-opacity': canDeleteConcessionDefinitive}" class="mx-1 simple-btn"><b-img :class="{'change-image-opacity': !canDeleteConcessionDefinitive}" src="@/assets/images/iconSUPPRIMER_16x16.png"></b-img>Supprimer</a>
                <a :disabled="!canViewDetailsForetCommunCD" @click.prevent="showDetails" size="sm" :class="{'change-image-opacity': !canViewDetailsForetCommunCD,  'not-change-image-opacity': canViewDetailsForetCommunCD}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconVISUALISER_16x16.png"></b-img>consulter</a>
...
export default {
  ...
  data: () => ({
    ...
    canViewDetailsSecteurs:true,
    canViewDetailsParcelles:true,
    ...
    fields: [
        ...
        { key: 'libelletypetitre', label: 'Type de titre',thStyle:{width:'9%'} },
        { key: 'numtitre', label: 'N° du titre',thStyle:{width:'2%'}},
        { key: 'entite', label: 'Entité Forestiere',thStyle:{width:'10%'}},
        { key: 'marteau', label: 'Marteau/sigleEtat',thStyle:{width:'2%'}},
        { key: 'valid', label: 'Statut',thStyle:{width:'2%'} },
  ...
  computed:{
    ...
    canCreateConcessionDefinitive(){ return true; return this.hasAccess('CREER_CONCESSION_DEFINITIVE')},
    ...
    canViewDetailsOperationParc(){return this.hasAccess('CONSULTER_CONCESSION_DEFINITIVE') },
    canViewDetailsForetCommunCD(){return true },
 ...
 methods: {
  showParcelles(){
    this.$router.push({name:'foret_communautaire_en_cd_details_parcelles',params: { id: this.selected[0].idtitre }})
  },
  showSecteurs(){
    this.$router.push({name:'foret_communautaire_en_cd_details_secteurs',params: { id: this.selected[0].idtitre }})
  },
  goToCreateTitreFCD(){this.$router.push({ name: "create-titre-fcd" });},
  showDetails(){
    this.$router.push({ name: 'foret_communautaire_en_cd_details' }); 
  },
  rowClass(item, type) {
    if(item !=''&&item!=null){
  ...
  onRowSelected(items) {
    ...
    if(this.selected && this.selected[0] && this.selected[0].valid!=="1"){
      ...
      this.canViewDetailsParcelles=this.canViewDetailsCCP=false
    }else if(this.selected && this.selected[0] && this.selected[0].valid==="1"){
      this.canViewDetailsParcelles=this.canViewDetailsCCP=true
    }
  },
  ...
  getTitres(){
    this.isBusy=true
     ...
     this.$titres.get("titres/FC", {params}).then(response => {
    ...
    filterTable() {
      ...
      this.isBusy=true
      this.$titres.post("titres/fc/search", this.search).then((response) => {
          this.total = response.data.result.totalItems
          this.currentPage = response.data.result.currentPage + 1
          this.elements=response.data.result.items
