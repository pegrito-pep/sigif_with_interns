<template>
    <div id="cover" class="min-vh-100">
          ...
          <div class="m-0 px-2 table-header-border">
            <b-row>
              <b-col>
                <h5 class="tab-header-left-text"> Liste des titres</h5>
              ...
              <b-col class="col-md-auto">
                <span class="d-flex">
                <b-button :disabled="!canCreateConcessionDefinitive" @click.prevent="goToCreateConcession" size="sm" :class="{'change-image-opacity': !canCreateConcessionDefinitive,  'not-change-image-opacity': canCreateConcessionDefinitive}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconPLUS_16x16.png"></b-img>Creer</b-button>
                <b-button :disabled="!canUpdateConcessionDefinitive" size="sm" :class="{'change-image-opacity': !canUpdateConcessionDefinitive,  'not-change-image-opacity': canUpdateConcessionDefinitive}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconMODIFIER_16x16.png"></b-img>Modifier</b-button>
                <b-button :disabled="!canDeleteConcessionDefinitive"  size="sm" :class="{'change-image-opacity': !canDeleteConcessionDefinitive,  'not-change-image-opacity': canDeleteConcessionDefinitive}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconSUPPRIMER_16x16.png"></b-img>Supprimer</b-button>
                <b-button :disabled="!canViewDetailsConcessionDefinitive" @click.prevent="showDetails" size="sm" :class="{'change-image-opacity': !canViewDetailsConcessionDefinitive,  'not-change-image-opacity': canViewDetailsConcessionDefinitive}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconVISUALISER_16x16.png"></b-img>consulter</b-button>
...
export default {
  ...
  computed:{
    ...
    canViewDetailsConcessionDefinitive(){return true; return this.hasAccess('CONSULTER_CONCESSION_DEFINITIVE') },
 ...
 methods: {
  showDetailsUFA(){
    this.$router.push({name:'concession_en_cd_UFA', params: { id: this.selected[0].idtitre }})
  },
  showDetailsUFE(){
    this.$router.push({name:'concession_en_cd_UFE', params: { id: this.selected[0].idtitre }})
  },
  showDetailsAAC(){
    this.$router.push({name:'concession_en_cd_AAC', params: { id: this.selected[0].idtitre }})
  },
  goToCreateConcession(){this.$router.push({ name: "create-titre-cd-convention" });},
  showDetails(){
    this.$router.push({name: 'concession_en_cd_detail', params: { id: this.selected[0].idtitre }});
  },
  rowClass(item, type) {
    if(item !=''&&item!=null){
  ...
  onRowSelected(items) {
    ...
    if(this.selected && this.selected[0] && this.selected[0].valid!=="1"){
      ...
      this.canViewDetailsAAC=this.canViewDetailsUFA=this.canViewDetailsUFE=false
    }else if(this.selected && this.selected[0] && this.selected[0].valid==="1"){
      this.canViewDetailsAAC=this.canViewDetailsUFA=this.canViewDetailsUFE=true
    }
  },
  ...
  getTitres(){
      this.isBusy=true
       ...
       this.$titres.get("titres/CD", {params}).then(response => {
    ...
    resetSearcheBar(){
     this.search = {
      ...
      localStorage.removeItem('sigif2./inventaires-concession_en_cd')
