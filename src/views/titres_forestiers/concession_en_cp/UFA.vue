                    ...
                    <b-row>
                        <b-col cols="6">
                            ...
                            >
                                ...
                                <input type="text" v-model="form.numUfa" v-if="showNewForm" class="w-50 mt-2  default-old-system-input-properties" :class="{'styleToSet':notSetNumUfa}"/>
                                <input v-else  type="text" disabled :value="selectedUfa ? selectedUfa.numUfa:''" class="w-50 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                ...
                                >
                                    <template #label>
                                        <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Superficie UFA(ha)
                                    </template>
                                    <input type="number" v-model="form.supufa" v-if="showNewForm" class="w-50 mt-2  default-old-system-input-properties" :class="{'styleToSet':notSetSupUfa}"/>
                                    <input disabled type="number" v-else :value="selectedUfa?(selectedUfa.superficieUfa):''" class="w-50 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                ...
                                >
                                    <template #label>
                                        <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Limites UFA(ha)
                                    </template>
                                    ...
                                    <b-link class="mx-1 detail-permis-item" v-else  target="_blank" :href="path+limitesUfa"><b-img src="@/assets/images/Attach_128x128.png"></b-img> {{limitesUfa}}{{ limitesUfa ? (!(limitesUfa.toString().includes('.')) ? '.pdf':'') :''}}</b-link>
                                </b-form-group>
                            </div>
                    ...
                    <b-row class="mt-0 p-0">
                        <b-col cols="6">
                            <b-table v-model="currentItemUfa"  ref="selectableTableufa" select-mode="single" responsive="sm" hover  selectable 
                                ...
                                <template #cell(limiteufa)="data">
                                    <span v-if="data.item!=null"><b-img src="@/assets/images/Attach_128x128.png"></b-img> &nbsp;{{ data.item.limiteufa }}</span>
                                </template>
                            </b-table>
                        </b-col>
                    </b-row>
...
export default{
    ...
    data(){
        return{
            ...
            fields:[
                {key:'numufa',label:'UFA n°'},
                {key:'superficie',label:'Superficie UFA(ha)'},
                {key:'limiteufa',label:'Limites UFA'},
            ],
            elements:[],
            form:{
                numUfa:'',
    ...
    methods:{
        ...
        async enregistrer(){
            ...
            try{
                ...
                // return;
                this.titreDetailsUfa.titreUfa=this.titreDetailsUfa.titreUfa.map(elt=>{
                    return{
                        numUfa:elt.numUfa, // string obligatoire
                        superficieUfa:elt.superficieUfa, // long
                        limiteufa:elt.limiteUfa, // base64 obligatoire
                ...
                //     }
                // })
                console.log('après ufas')
                await this.$titres.post('titres/ufa',{
                    ...
                    this.showOverlay=false
                    let message={ message:'UFA créée avec succès'}
        ...
        async getInitialData() {
            ...
            this.titreDetailsUfa = await this.$titres.get('titre_convenssion/detailcpUfa/' +this.$route.params.id).then(response => response.data.result)
            this.elementssuperficie2=[]
        ...
        onRowSelected(items) {
            ...
            if(!this.showNewForm || this.canSave){
                ...
                this.selectedUfa = this.titreDetailsUfa.titreUfa.find(elt=>elt.numUfa===items[0]?.numufa && elt.limiteUfa.split('/').pop()===items[0].limiteufa)
                if(this.selectedUfa){
                    this.selectedUfa.numUfa=items[0]?.numufa
                    this.selectedUfa.superficieUfa=items[0]?.superficie
