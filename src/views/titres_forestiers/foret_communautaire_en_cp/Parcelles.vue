        ...
        <b-overlay :show="showOverlay" rounded="sm">
            <div class="m-0">
                <tab-head
                    ...
                    :nouveau="nouveau"
                    :canAddNew="canAddNew"
                    title="Parcelle de la forêt communautaire n°"
                    :idElement="idElement"
                >
                </tab-head> 
                    ...
                    <b-row>
                        <b-col cols="6">
                            <b-form-group class="p-0 m-0"
                                ...
                                label-align="right"
                            >
                                <label class="label-form" v-if="titreDetailsUfa!=null">{{ (titreDetailsUfa.superficietitre) }}</label>
                            </b-form-group>
                            <b-form-group class="p-0 m-0"
                                label="Titre n°"
                                label-cols="4"
                                label-align="right"
                            >
                                <label class="label-form" v-if="titreDetailsUfa!=null">{{ titreDetailsUfa.numtitre }}</label>
                            </b-form-group>
                            <b-form-group class="p-0 m-0"
                                ...
                                label-align="right"
                            >
                                <label class="label-form" v-if="titreDetailsUfa!=null">{{ titreDetailsUfa.entiteforestierer }}</label>
                            </b-form-group>
                            <b-form-group class="p-0 m-0"
                                label="Marteau"
                                label-cols="4"
                                label-align="right"
                            >
                                <label class="label-form" v-if="titreDetailsUfa!=null">{{ titreDetailsUfa.marteau }}</label>
                            </b-form-group>
                            <div >
                                <b-form-group class="p-0 m-0"
                                label="Parcelle"
                            ...
                            >
                                <template #label>
                                    <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Parcelle
                                </template>
                                <input type="text" v-model="form.numUfa" v-if="showNewForm" class="w-20 mt-2  default-old-system-input-properties" :class="{'styleToSet':notSetNumUfa}"/>
                                <input v-else  type="text" disabled :value="selectedUfa ? selectedUfa.parcelle:''" class="w-20 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                ...
                                >
                                    <template #label>
                                        <span v-if="showNewForm" style="color:red;font-weight: bold;">*</span>Superficie de la parcelle
                                    </template>
                                    <input type="number" v-model="form.supufa" v-if="showNewForm" class="w-20 mt-2  default-old-system-input-properties" :class="{'styleToSet':notSetSupUfa}"/>
                                    <input disabled type="number" v-else :value="selectedUfa?(selectedUfa.superficieParcelle):''" class="w-20 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                ...
                                >
                                    ...
                                    <input type="text" v-model="form.annee" v-if="showNewForm" class="w-20 mt-2  default-old-system-input-properties" :class="{'styleToSet':notSetLimitesUfa}"/>
                                    <input v-else  type="text" disabled :value="selectedUfa ? selectedUfa.anneExploittion:''" class="w-20 mt-2  default-old-system-input-properties" style="background-color:#eee;"/>
                                    <!-- <b-form-file v-if="showNewForm || !isUfa" v-model="form.limites" @change="getFileBase64" class="default-old-system-input-properties-file" :class="{'styleToSet':notSetLimitesUfa}"></b-form-file>
                                    <a class="mx-1 detail-permis-item" v-else v-b-tooltip.hover  target="_blank" :href="titreDetailsUfa.limiteufa"><b-img src="@/assets/images/Attach_128x128.png"></b-img> {{limitesUfa}}{{ limitesUfa ? (!(limitesUfa.toString().includes('.')) ? '.pdf':'') :''}}</a> -->
                                </b-form-group>
                            </div>
                        ...
                        <b-col>
                            <b-row>
                                <b-col cols="8">
                                    <h6 class="pt-1" style="font-weight: bold;border-bottom:1px solid black;">Superficie de la parcelle 
                    ...
                    <b-row class="mt-0 p-0">
                        <b-col cols="5">
                            <b-table v-model="currentItemUfa"  ref="selectableTableufa" select-mode="single" responsive="sm" hover  selectable 
                                ...
                                <template #cell(superficie)="data">
                                    <span>{{ formatAmount(data.item.superficie) }}</span>
                                </template>
                            </b-table>
                        </b-col>
                    </b-row>
...
export default{
    ...
    //   },
      ...
      "form.supufa"(value){
        if(!php.empty(value)){
            this.notSetSupUfa=false
        }else{
            this.notSetSupUfa=true
        }
      },
      "form.annee"(value){
        if(!php.empty(value)){
            this.notSetLimitesUfa=false
        }else{
            this.notSetLimitesUfa=true
        }
      },
  },
    methods:{
        checkForm(){
            if(this.showNewForm){
                ...
                if(php.empty(this.form.supufa)){
                    this.notSetSupUfa=true
                    let error={ message:'Veuillez renseigner la superificie de l\'UFA'}
                    this.showOverlay=false
                    return this.sendAlertMessage(error);
                }
                if(php.empty(this.form.limites)){
                    this.notSetLimitesUfa=true
                    let error={ message:'Veuillez renseigner les limites de l\'UFA'}
                    this.showOverlay=false
                    return this.sendAlertMessage(error);
                }
                if(this.elementssuperficie2.length===0 || this.elementssuperficie2.some(x=>php.empty(x.arrondisement) && php.empty(x.superficie))){
                    let error={ message:'Veuillez renseigner la localisation de l\'UFA'}
                    this.showOverlay=false
                    return this.sendAlertMessage(error);
                }
                this.disabledEnregistrer=false
            }
        },
        ...
        async enregistrer(){
            ...
            if(this.showNewForm){
                ...
                if(this.elementssuperficie2.length===0){
            ...
            try{
                ...
                // return;
                this.titreDetailsUfa.titreaac=this.titreDetailsUfa.titreaac.map(elt=>{
                    return{
                        numAac:elt.parcelle, // string obligatoire
                        superficieAssiete:elt.superficieParcelle, 
                        anneeExploitation:elt.anneExploittion,// long
                        superficie:superficies
                    }
                })
                this.ufas.push({
                    numAac:this.form.numUfa,
                    superficieAssiete: Number(this.form.supufa),
                    anneeExploitation:Number(this.form.annee),
                    superficie:superficies,
                })
                console.log('aacs',this.ufas)
                console.log('après aacs ')
                await this.$titres.post('foretCommunautaire/parcelle',{
                    ...
                    aacs:this.ufas
                        ...
                        superficie: this.form.supufa
                    })
                    // console.log(response)
                    // App.notifySuccess(response.data.message)
                    this.showOverlay=false
                    let message={ message:'Parcelle créée avec succès'}
        ...
        ajouter(){
            ...
            if(php.empty(this.form.numUfa)){
                this.notSetNumUfa=true
                let error={ message:'Veuillez renseigner la parcelle'}
                return this.sendAlertMessage(error);
            }
            if(php.empty(this.form.supufa)){
                this.notSetSupUfa=true
                let error={ message:'Veuillez renseigner la superificie'}
                return this.sendAlertMessage(error);
            }
        ...
        nouveau(){
            ...
            if(this.showNewForm){
                ...
                this.elementsufa.unshift({
                    numufa: this.form.numUfa,
                    superficie: this.form.supufa,
                    limiteufa: this.form.limites.name,
                })
                this.canSave=true
                this.disabledEnregistrer=false
        ...
        async getInitialData() {
            ...
            this.titreDetailsUfa = await this.$titres.get('foretCommunautaire/parcelle/' +this.$route.params.id).then(response => response.data.result)
            this.elementssuperficie2=[]
            this.idElement=this.titreDetailsUfa.numtitre
            if(this.titreDetailsUfa.titreaac.length===0){
                this.isUfa=false
            }else{
                this.isUfa=true
            }
            if(this.isUfa){
                superficie2=this.elementssuperficie.reduce((acc,val)=>acc+val.superficie,0)
            }
            console.log(this.titreDetailsUfa.titreaac)
            if(this.titreDetailsUfa.titreaac.length!==0) {
                this.elementsufa=this.titreDetailsUfa.titreaac.map(elt=>{
                    return{
                        numufa:elt.parcelle,
                        superficie:elt.superficieParcelle,
                    }
                })
                const numUfas=this.elementsufa.map(elt=>elt.numufa)
                this.selectedUfa = this.titreDetailsUfa.titreaac.find(elt=>elt.parcelle===numUfas[0])
                // let tableau=[]
                // tableau=this.selectedUfa?.limiteUfa?.split('/')
                // if(!php.empty(tableau)){
                //     this.limitesUfa=tableau[tableau.length -1]
                // }
                
                console.log('elmtUfas',this.elementsufa)
                console.log('selectedufa',this.selectedUfa)
                console.log('numufas',this.numUfas)
            ...
            let dataformatted = {
                numufa: this.titreDetailsUfa.numufa,
                superficie: superficie2,
                limiteufa: this.limitesUfa,
            }
            // if(this.titreDetailsUfa?.numufa) this.titreDetailsUfa.superficie2=superficie2
        ...
        onRowSelected(items) {
            ...
            if(!this.showNewForm || this.canSave){
                console.log('row selected',items);
                this.selectedUfa = this.titreDetailsUfa.titreaac.find(elt=>elt.parcelle===items[0]?.numufa)
                if(this.selectedUfa){
                    this.selectedUfa.parcelle=items[0]?.numufa
                    this.selectedUfa.superficieParcelle=items[0]?.superficie
                    // this.selectedUfa.limiteUfa=items[0]?.limiteufa
                    // this.limitesUfa=items[0]?.limiteufa
