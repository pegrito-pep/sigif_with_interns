          ...
          <div class="m-0 px-2 table-header-border">
            <b-row>
              <b-col>
                ...
                <b-button @click.prevent="showParcelles"  size="sm" :class="{'change-image-opacity': !canViewDetailsParcelles,  'not-change-image-opacity': canViewDetailsParcelles}" class="mx-1 simple-btn"><b-img src="@/assets/images/Parcs_Rupture_16x16.png"></b-img>Parcelles</b-button>
                <!-- <a :disabled="canCreateUtilisateur" @click.prevent="addUser" size="sm" :class="{'change-image-opacity': !canCreateUtilisateur,  'not-change-image-opacity': canCreateUtilisateur}" class="mx-1 simple-btn"><b-img  src="@/assets/images/Picto_tracabilite/Parcs_Usine_16x16.png"></b-img> UFE</a>
                ...
                <b-button @click.prevent="" size="sm" :class="{'change-image-opacity': !canViewDetailsCCP,  'not-change-image-opacity': canViewDetailsCCP}" class="mx-1 simple-btn"><b-img  src="@/assets/images/Picto_tracabilite/iconINVENTAIRE_16x16.png"></b-img> Contrat de partenariat</b-button>
              </b-col>
              <b-col class="col-md-auto">
                <span class="d-flex">
                <a :disabled="!canCreateConcessionDefinitive" @click.prevent="goToCreateTitreFCP" size="sm" :class="{'change-image-opacity': !canCreateConcessionDefinitive,  'not-change-image-opacity': canCreateConcessionDefinitive}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconPLUS_16x16.png"></b-img>Creer</a>
                <a :disabled="canUpdateConcessionDefinitive" @click.prevent="showDetails" size="sm" :class="{'change-image-opacity': !canUpdateConcessionDefinitive,  'not-change-image-opacity': canUpdateConcessionDefinitive}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconMODIFIER_16x16.png"></b-img>Modifier</a>
                <a :disabled="canDeleteConcessionDefinitive" @click.prevent="showDetails" size="sm" :class="{'change-image-opacity': !canDeleteConcessionDefinitive,  'not-change-image-opacity': canDeleteConcessionDefinitive}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconSUPPRIMER_16x16.png"></b-img>Supprimer</a>
                <a :disabled="canViewDetailsForetCommunCP" @click.prevent="showDetails" size="sm" :class="{'change-image-opacity': !canViewDetailsForetCommunCP,  'not-change-image-opacity': canViewDetailsForetCommunCP}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconVISUALISER_16x16.png"></b-img>consulter</a>
                <a  size="sm"  class="not-change-image-opacity mx-1 simple-btn d-flex">
                  <span class="mr-1">Sélectionner une action</span>
...
export default {
  ...
  data: () => ({
    ...
    fields: [
        ...
        { key: 'libelletypetitre', label: 'Type de titre',thStyle:{width:"8%"} },
        { key: 'numtitre', label: 'N° du titre',thStyle:{width:"2%"} },
        { key: 'entite', label: 'Entité Forestiere',thStyle:{width:"10%"} },
        { key: 'marteau', label: 'Marteau',thStyle:{width:"1%"} },
        { key: 'valid', label: 'Statut',thStyle:{width:"1%"} },
  ...
  computed:{
    ...
    canViewDetailsForetCommunCP(){return true },
    // canViewDetailsForetCommunCP(){return this.hasAccess('LIST_FORET_COMMUNAUTAIRE_EN_CP') },
    canCreateUtilisateur(){return true ;return this.hasAccess('CONSULTER_CONCESSION_DEFINITIVE') },
 ...
 methods: {
  showParcelles(){
    this.$router.push({name:'foret_communautaire_en_cp_details_parcelles',params: { id: this.selected[0].idtitre }})
  },
  goToCreateTitreFCP(){this.$router.push({ name: "create-titre-fcp" });},
  showDetails(){
    this.$router.push({name: 'foret_communautaire_en_cp_details'}); 
  },
  rowClass(item, type) {
    if(item !=''&&item!=null){
  ...
  getTitres(){
    this.isBusy=true
     ...
     this.$titres.get("titres/FS", {params}).then(response => {
    ...
    filterTable() {
      ...
      this.isBusy=true
      this.$titres.post("titres/fs/search", this.search).then((response) => {
          this.total = response.data.result.totalItems
          this.currentPage = response.data.result.currentPage + 1
          this.elements=response.data.result.items
