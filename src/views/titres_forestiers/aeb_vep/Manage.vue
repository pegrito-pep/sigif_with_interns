          ...
          <div class="m-0 px-2 table-header-border">
            <b-row>
              ...
              <b-col class="col-md-auto">
                <span class="d-flex">
                <a :disabled="!canCreateTitreAebVep" @click.prevent="goToCreateAebVep" size="sm" :class="{'change-image-opacity': !canCreateTitreAebVep,  'not-change-image-opacity': canCreateTitreAebVep}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconPLUS_16x16.png"></b-img>Creer</a>
                ...
                <a :disabled="canViewDetailsAEB" @click.prevent="showDetails" size="sm" :class="{'change-image-opacity': !canViewDetailsAEB,  'not-change-image-opacity': canViewDetailsAEB}" class="mx-1 simple-btn"><b-img src="@/assets/images/iconVISUALISER_16x16.png"></b-img>consulter</a>
...
export default {
  ...
  computed:{
    ...
    canCreateTitreAebVep(){ return true; return this.hasAccess('CREER_CONCESSION_DEFINITIVE')},
 ...
 methods: {
  goToCreateAebVep(){this.$router.push({ name: "create-titre-aebvep" });},
  showDetails(){
    this.$router.push({name: 'aeb_vep-detail'}); 
  },
  rowClass(item, type) {
    if(item !=''&&item!=null){
  ...
  getTitres(){
     this.isBusy=true
      ...
      this.$titres.get("titres/ae", {params}).then(response => {
    ...
    filterTable() {
      ...
      this.isBusy=true
      this.$titres.post("titres/ae/search", this.search).then((response) => {
          this.total = response.data.result.totalItems
          this.currentPage = response.data.result.currentPage + 1
          this.elements=response.data.result.items
