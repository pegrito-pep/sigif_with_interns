            ...
            <b-row class="mt-2">
              <b-col cols="5">
                <b-row class="my-2">
                  ...
                  <b-col cols="7">
                    <select disabled :class="{ styleToSet: notSetTypeInventaire }" class="m-0 p-0 default-old-system-input-properties w-100"   style="width: 50%;font-size:1rem; border-radius:0.25rem; height: calc(1em + .200rem + 5px) !important;">
                      <option selected>Autorisation de récupération du bois</option>
                    </select>
                  </b-col>
                </b-row>
                ...
                <b-row class="my-2">
                  ...
                  <b-col cols="7">
                    <input class="default-old-system-input-properties" :class="{ styleToSet: notSetNumCatalogue }" v-model="form.numcatalogue"  size="sm"  placeholder="" type="text" style="width:50%!important"/>
                  </b-col>
                </b-row>
                <b-row class="my-2">
                ...
                <b-row class="my-2">
                  <b-col cols="5" class="d-flex justify-content-end"><h6 class="font-weight-bold"><span style="color: red; font-size:1.5rem">*</span> Nom du promoteur</h6></b-col>
                  <b-col cols="7">
                    <input class="system-input-nom-promoteur" :class="{ styleToSet: notSetPromoteur }" v-model="form.promoteur"  size="sm"  placeholder="" type="text"/>
                  </b-col>
                </b-row>
                <b-row class="my-2">
                  <b-col cols="5" class="d-flex justify-content-end"><h6 class="font-weight-bold"><span style="color: red; font-size:1.5rem">*</span>Etude d'impact</h6></b-col>
                  <b-col cols="7">
                    <textarea rows="1" cols="25" class="w-100" :class="{ 'styleToSet': notSetEtudeImpact }" type="text" v-model="form.etudeimpact" ></textarea>
                  </b-col>
                </b-row>

                <b-row class="my-2">
                  <b-col cols="5" class="d-flex justify-content-end"><h6 class="font-weight-bold">Superficie</h6></b-col>
                  <b-col cols="7">
                    <input class="system-input-nom-promoteur" v-model="form.superficie"  size="sm"  placeholder="" type="text"/>
                  </b-col>
                </b-row>
                ...
                <b-row class="my-2">
                  ...
                  <b-col cols="7">
                    <b-form-file id="rapportinventairefile" size="lg" v-model="form.fichierInventaire" @change="getBase64FileInventaire" class="catalogue-form-file" :class="{ 'styleToSet': notSetFichierInventaire }" plain></b-form-file>
                  </b-col>
                </b-row>
                
                <b-row class="my-2">
                  <b-col cols="5" class="d-flex justify-content-end"><h6 class="font-weight-bold"><span style="color: red;">*</span>Etude d'impact</h6></b-col>
                  <b-col cols="7">
                      <b-form-file id="etudeimpactfile" size="sm" v-model="form.fichierImpact" @change="getBase64FileEtudeImpact" :class="{ 'styleToSet': notSetFichierEtudeImpact }" plain></b-form-file>
                  </b-col>
                </b-row> 
              </b-col>
              <b-col cols="7">
                <div style="height: 300px; overflow-y: auto; overflow-x: hidden;">
                  <div class="table-header-border">
                    <b-row>
                        ...
                        <b-col>
                          <a @click.prevent="addFielLocalisationARB" class="mx-1 simple-btn" style="cursor: pointer"><b-img  src="@/assets/images/iconPLUS_16x16.png"></b-img>Ajouter</a>
                  ...
                  <b-overlay rounded="sm" :show="showOverlayLocalisation">
                    <b-table :items="itemsLocalisationARB"  :fields="fieldsLocalisationARB" class="ml-1 px-2 mt-1" outlined   ref="selectableTablegrumes" select-mode="single" selectable @row-selected="onRowSelectedLocation" :tbody-tr-class="rowClass">
                      ...
                      <template #cell(superficie)="data">
                        <input  size="xs" class="mx-1 px-1 sigif-input-view text-left"  style="color:black!important; width:80%" v-model="data.item.superficie" />
                      </template>
            ...
            <div>
              <div class="mx-4 px-2 table-header-border mt-2">
                <b-row>
                    ...
                    <b-col class="liens">
                      ...
                      <a @click.prevent="addFieldProduitARB" class="mx-2 simple-btn"><b-img  src="@/assets/images/iconPLUS_16x16.png"></b-img>Ajouter</a>
                      <a @click.prevent="remouveAllFieldsProduitsARB"  class="mx-2 simple-btn"><b-img class="mr-1" src="@/assets/images/iconSUPPRIMER_16x16.png"></b-img>Tout supprimer</a>
                    </b-col>
                </b-row>
              </div>
              ...
              <div class="">
                <b-overlay :show="showOverlayCodeData" rounded="sm">
                  <b-table max-height="700px" v-model="currentItemGrume" :items="itemsARB"  
                    :fields="fieldsproduitsARB" :busy="isBusy" class="ml-4 px-2 mt-1" outlined   
                    ref="selectableTableDetailLot" select-mode="single" selectable 
                    @row-selected="onRowSelectedProduitsARB" :tbody-tr-class="rowClass">
                  ...
                  <paginator
                    ...
                    v-show="itemsProduitsARB.length > 25"
                  />
                </b-overlay>
              </div>
            </div>
...
import FileComponent from '@/components/utils/inventaire/ImportCatalogueFile.vue';


export default {
  name:"create-catalogue-arb",
  components:{ErrorDialogue, CodeBarreInfo, ApiUnreachable, FileComponent},
   watch: {
    ...
    "form.etudeimpact"(value) {
      if(!php.empty(value)){
          this.notSetEtudeImpact= false
        }
        else{
          this.notSetEtudeImpact= true
        }
    },
    ...
    "form.etudeimpactfichier"(value) {
      if(!php.empty(value)){
          this.notSetFichierEtudeImpact= false
        }
        else{
          this.notSetFichierEtudeImpact= true
        }
    },
  ...
  methods: {
    ...
    handleDataImported(data){
      for(let i=0;i<data.length; i++){
        this.elementsproduitsARB.push(data[i])
      }
 
    this.$refs.filecomponent._close();
      
     /* this.elementsproduitsARB.map(elt =>{
        elt.volume = elt.volume!=""?elt.volume.toFixed(3):""
        return elt
      })*/
    },
    remouveAllFieldsProduitsARB(){
      this.elementsproduitsARB = [];
    },
    addFieldProduitARB(){
          ...
          if (php.empty(this.form.fichierImpact)) {
            this.notSetFichierEtudeImpact=true;
            let error={ message:'Veuillez renseigner le fichier de l\'étude d\'impact'}
            return this.sendAlertMessage(error);
          }
          //controle du fait qu'au moin une localisaion soit ajoutée
          if(php.empty(this.elementsLocalisationARB)){
            let error={ message:'Veuillez ajouter au moins une localisation'}
            return this.sendAlertMessage(error);
          }
          else if(!php.empty(this.elementsLocalisationARB)){
            if(php.empty(this.elementsLocalisationARB[0].idarrondissement) ||php.empty(this.elementsLocalisationARB[0].idarrondissement)){
    ...
    async getBase64FileEtudeImpact(e) {
      ...
      console.log(!php.empty(this.form.etudeimpactfichier));
      console.log('1',this.form.etudeimpactfichier)

		},
    RemouveFieldProduit(index){
      this.elementsproduitsARB.splice(index, 1);
    },
    ...
    onRowSelectedProduitsARB(items){
      console.log('row selected',items);
      items.isSelected =true;
    },
    ...
    async loadOtherDataLocation(index,data){
      console.log('data',data);
        ...
        try{
          if(php.empty(data)){
            this.elementsLocalisationARB[index].departement='';
            this.elementsLocalisationARB[index].region='';
          ...
          this.arrondissement=await this.$donneesReference.get(`arrondissements/${data}/departement_region`)
            .then(resp=>{
              ...
              this.elementsLocalisationARB[index].departement=arr.departement;
              this.elementsLocalisationARB[index].region=arr.region;
            })
        }catch(err){
    ...
    addFielLocalisationARB(){
        ...
        this.elementsLocalisationARB.unshift({idarrondissement: '', departement: '', region : '', localisation:'',superficie:''});  
    },
    ...
    formatPostContent(){      
      if (php.empty(this.form.numcatalogue)) {
        ...
        this.elementsLocalisationAEB = this.elementsLocalisationAEB.map(elt =>{
            elt.superficie=120000              
            return elt
        })
        this.form.superficieDto=this.elementsLocalisationAEB
        this.elementsproduitsAEB = this.elementsproduitsAEB.map(elt =>{
            elt.volume=1              
            return elt
        })
        this.form.produitboar=this.elementsproduitsAEB
        return true;
    },
    async sendFormCatalogue(){
      ...
      if(this.formatPostContent){
        ...
        data.promoteur = this.form.promoteur;
        data.etudeimpact = this.form.etudeimpact;
        data.superficie = this.form.superficie;
        data.rapportinventaire = this.form.rapportinventaire;
        data.etudeimpactfichier = this.form.etudeimpactfichier;
        data.superficieDto = this.elementsLocalisationARB; 
        data.produitboar = this.elementsproduitsARB;
        console.log('data ',data);
    ...
    async getSequence(index, value){
      ...
      async showDialogueApiError(message){
        ...
        if (ok) {
          ...
          this.$refs.codebarreinfo._close();
          this.elementsproduitsAEB[0].codebarre=''
          // this.$router.back();
        }
      },
  ...
  computed:{
    ...
    valuesSetCorrectly() {
      ...
      return ( !php.empty(this.form.numcatalogue)&&!php.empty(this.form.typetitre)&&!php.empty(this.form.typeinventaire)
              &&!php.empty(this.form.exercice)&&!php.empty(this.form.promoteur)&&!php.empty(this.form.etudeimpact)
              &&(document.getElementById("rapportinventairefile").files.length > 0 ) &&(document.getElementById("etudeimpactfile").files.length > 0 )
              &&!php.empty(this.elementsLocalisationARB)&&!php.empty(this.elementsLocalisationARB[0].idarrondissement)
              &&!php.empty(this.elementsLocalisationARB[0].localisation)&&!php.empty(this.elementsLocalisationARB[0].superficie)&&
              !php.empty(this.elementsproduitsARB)&&
              !php.empty(this.elementsproduitsARB[0].codebarre)&&!php.empty(this.elementsproduitsARB[0].diametreTige)&&!php.empty(this.elementsproduitsARB[0].volume)
      );
    },
